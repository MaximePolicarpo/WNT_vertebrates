...
blast_rslt <- read.table("Prot_vs_Genome.blastn", header=FALSE, sep="\t")
...
V2R_best_hits <- scan(file="Prot_Regions.tsv", what="character")


Best_hits_filtered <- Best_hits_filtered %>% filter(samtools_name %in% V2R_best_hits)


#remove regions of already found V2R genes
...
write(Best_hits_filtered$samtools_name, file="Potential_regions.tsv")
