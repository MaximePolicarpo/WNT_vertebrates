...
for(putative_protein_length in c(1)){ 

	miniprot_rslt_round <- miniprot_rslt_perf %>% filter(perc_aligned >= putative_protein_length)
...
miniprot_rslt_perf <- 
  miniprot_rslt %>%
  filter(grepl("Identity=0.9", desc))
